...
linethickness = floor(.02*width_s); 
...
for idx=1:length(files)
    ...
    for i=1:length(s)
        ...
        % And remove them
        newgrapheme=imcomplement(imbinarize(rgb2gray(grapheme),1.2*graythresh(rgb2gray(grapheme))));
        %% Step 1 : Hough transform to find lines (needs optimization)
        [H,T,R] = hough(newgrapheme);
        P  = houghpeaks(H,64,'threshold',ceil(0.5*max(H(:))));
